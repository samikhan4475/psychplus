...
import { AlertCircle, CheckCircle, InfoIcon, XIcon } from 'lucide-react'
...
const ToastProvider = ({ children }: React.PropsWithChildren) => {
  ...
  const toastValue = useMemo(() => ({ toast }), [])
...
const renderToastType = (toastType: ToastData['type']) => {
  return (
    ...
    >
      {toastType === 'success' && (
        <CheckCircle width={20} height={20} className="text-green-10" />
      )}
      {toastType === 'error' && (
        <AlertCircle width={20} height={20} className="text-red-10" />
      )}
      {toastType === 'info' && (
        <InfoIcon width={20} height={20} className="text-blue-10" />
      )}
    </Flex>
  )
}
