...
const FavouriteDiagnosis = () => {
  ...
  const handleValueChange = async (favorite: FavouriteDiagnosisData) => {
    const isValueAlreadyExist = workingDischargeDiagnosisData.some(
    ...
    updateWorkingDischargeDiagnosisData(data)
  }
      ...
      <Table.Root>
        <Table.Body className="align-middle">
          {favouriteDiagnosisData.map((item, index) => (
            <Table.Row key={`${item.icd10Code}-${index}`}>
              <Table.Cell
