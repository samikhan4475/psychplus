...
const CptCodeSelect = () => {
  ...
  const onSelect = (item: SearchItem) => {
    const existingCodes = form.getValues('customCptCodes')?.trim() || ''
    const separator = existingCodes ? ',' : ''
    const updatedCodes = `${existingCodes}${separator}${item.code}`

    form.setValue('customCptCodes', updatedCodes, {
      shouldDirty: true,
    })
  }
  const onRemoveCptCode = (code: string) => {
    const alreadyExistingCodes = form.getValues('customCptCodes')
  ...
  const customCptCodes = transformInCodes(form.watch('customCptCodes') ?? '')
  const disabledCodes = customCptCodes.concat(
    transformInCodes(form.watch('customAddons') ?? ''),
  )
  return (
    <Flex direction="column" className="shadow-2">
      ...
      <FormFieldError name="customCptCodes" />
    </Flex>
  )
}
