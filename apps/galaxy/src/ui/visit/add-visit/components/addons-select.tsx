...
const AddonsSelect = () => {
  ...
  const onSelect = (item: SearchItem) => {
    const existingCodes = form.getValues('addOnCodes')?.trim() || ''
    const separator = existingCodes ? ',' : ''
    const updatedCodes = `${existingCodes}${separator}${item.code}`

    form.setValue('addOnCodes', updatedCodes)
  }
  const onRemoveAddonCode = (code: string) => {
    const alreadyExistingCodes = form.getValues('addOnCodes')
  ...
  const addOnCodes = transformInCodes(form.watch('addOnCodes') ?? '')
  const disabledCodes = addOnCodes.concat(
    transformInCodes(form.watch('cptCodes') ?? ''),
  )
  return (
