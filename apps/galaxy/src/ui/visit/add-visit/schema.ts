...
const customVisitRequiredFields = (flagEnabled: boolean) =>
  flagEnabled
    ? {
...
const schema = (customVisitFeatureEnabled: boolean) =>
  z
    ...
    .extend(customVisitRequiredFields(customVisitFeatureEnabled))
        ...
        if (data.dischargeDate && data.dateOfAdmission && data.visitSequence) {
          ...
          if (
            ...
            _ctx.addIssue({
              ...
              path: ['dischargeDate'],
              message:
                'Discharge date cannot be greater than date of admission',
            })
          }
        }
      }
