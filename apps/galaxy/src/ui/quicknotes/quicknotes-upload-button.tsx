...
const QuickNotesUploadButton = ({ isWhiteBg = false, isIcon = true }: QuickNotesUploadButtonProps) => {
  ...
  const uploadFiles = async (files: FileList): Promise<boolean> => {    
    ...
    toast.error(
      response.error ?? 'There was a problem uploading the document. Please try again.'
    );
    return false;
  };
