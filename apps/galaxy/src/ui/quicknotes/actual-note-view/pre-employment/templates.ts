...
((patientName)) is a ((patientAge)) year old ((gender|lower)) who is ((relationshipStatus|lower)) and currently living in ((currentCity)), ((livingArrangement|lower)). ((patientName)) was born in ((placeOfBirth)) and raised ((placeRaised)).
((familyHistoryDetails)).The subject’s previous employer is ((previousEmployer)), they worked there for ((durationAtPreviousEmployer)). Prior to that, they ((priorEmployment)). The subject ((hasDisciplinaryActions|lower)) had disciplinary write-ups/actions while employed((disciplinaryIncidentDescription|prependColonIfPresent)). ((useOfForceOrComplaints))<<BREAK>>
...
((he)) appears ((socialAdjustment|lower)) adequately adjusted socially, both in terms of family history, and recent and remote personal history. ((patientName)) ((legalHistory|fixHave)) a history of legal difficulties((hasLegalHistory|prependColonIfPresent)). They ((hasRestrainingOrder|lower)) been the subject of a restraining order((restrainingOrderDetails|prependColonIfPresent)). The subject ((hasCivilLitigation|lower)) been involved in civil litigation((civilLitigationDetails|prependColonIfPresent)). Their motor vehicle record is ((motorVehicleRecord))((motorVehicleRecordDetails|prependColonIfPresent)).<<BREAK>>
