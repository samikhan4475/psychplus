...
import { NOTE_SIGN_ENDPOINT } from '@/api/endpoints'
...
const signNoteClientAction = async (
  ...
  let url = NOTE_SIGN_ENDPOINT(patientId, appointmentId)
  if (isError) {
    url += `?errormark=true`
  }
  ...
  const response = await api.PUT<QuickNoteSectionItem[]>(url, payload)
