    ...
    <>
      ...
      <Flex className="p-3 pt-0" direction="column">
        <Table.Body>
          ...
          <Table.Row>
            {heading.map((heading, colIndex) => {
              const fieldKey = `DelayedRecall${heading}Q1`
          ...
          <Table.Row>
            {heading.map((heading, colIndex) => {
              const fieldKey = `DelayedRecall${heading}Q2`
              const isFirstColumn = colIndex === 0
              return (
                <Table.Cell
                  key={`col-${heading}-${colIndex}`}
        ...
        <Text size="1" className="pt-3">
          1 point per word recalled without cue
        </Text>
      </Flex>
    </>
  )
