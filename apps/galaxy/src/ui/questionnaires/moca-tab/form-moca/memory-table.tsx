        ...
        <Table.Body>
          ...
          <Table.Row>
            {heading.map((heading, colIndex) => {
              const fieldKey = `Memory${heading}Q1`
              const isFirstColumn = colIndex === 0
              return (
                <Table.Cell
                  key={`col-${heading}-${colIndex}`}
                  className={cn(
                    'border-pp-table-border h-5 border-b border-l border-r-0 px-1 py-0.5 last:border-r',
                    {
                      'bg-pp-focus-bg-2': isFirstColumn,
                    },
                  )}
                >
                  {isFirstColumn ? (
                    <Text size="1">1st Trial</Text>
                  ) : (
                    <Flex>
                      <CheckboxCell
          ...
          <Table.Row>
            {heading.map((heading, colIndex) => {
              const fieldKey = `Memory${heading}Q2`
              const isFirstColumn = colIndex === 0
              return (
                <Table.Cell
                  key={`col-${heading}-${colIndex}`}
