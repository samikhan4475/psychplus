...
const getOrganizationStaffTypesOptionsAction = async (): Promise<
...
> => {
  ...
  response.data?.forEach(({ users }) => {
    users?.forEach(({ userRoles }) => {
      userRoles?.forEach(({ actorCategory, id }) => {
        roleLookup[id] = { label: actorCategory, value: id }
      })
    })
  })
...
export { getOrganizationStaffTypesOptionsAction }
