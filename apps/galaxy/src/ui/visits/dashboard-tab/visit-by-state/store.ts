...
import { StateVisitsListResponse, StateVisitsParams } from '../../types'
import { getDateRangeFromFilter } from '../../utils'
import { getVisitByStateListAction } from '../actions/get-visit-by-state-list'

interface Store {
  data?: StateVisitsListResponse
  loading?: boolean
  error?: string
  activeFilter?: string
  ...
  page: number
  sort?: Sort
  pageCache: Record<number, StateVisitsListResponse>
...
const useStore = create<Store>((set, get) => ({
  ...
  search: async (
    ...
    const result = await getVisitByStateListAction({
      payload,
      sort: get().sort,
      page,
    })
