...
const columns: ColumnDef<PatientReferral>[] = [
  ...
  {
    id: 'isPatientEducation',
    header: () => <ColumnHeader label="Patient Education" />,
    cell: ({ row: { original } }) => (
      <TextCell className="truncate">
        {original.isPatientEducation ? 'Yes' : 'No'}
      </TextCell>
    ),
  },
  {
    ...
    header: ({ column }) => <ColumnHeader column={column} label="Created On" />,
...
const IntReferralsHistoryEducationCellTable = ({
  referralId,
    ...
    getPatientReferralsHistoryAction({ referralId }).then((response) => {
      if (response.state === 'error') {
        ...
        const compiedData = response?.data?.filter((item) => {
          if (item?.isPatientEducation !== prev) {
            prev = item?.isPatientEducation
            return true
          }
          return false
        })
...
export { IntReferralsHistoryEducationCellTable }
