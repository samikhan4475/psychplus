...
import z from 'zod';
import { addScheduleReportAction, runScheduleReportAction } from '../actions';
import { useStore } from '../store';
import { REPEAT_INTERVAL } from '../types';
import { formatJobData, generateCronExpression, processParameters } from '../utils';
...
const schema = z.object({
  beginOn: z.custom<DateValue>().refine((value) => value !== null && value !== undefined, {
    message: 'Required',
  }),
  ...
  parameters: z.array(
    z.object({
      id: z.string(),
      scheduleParameterValue: z.union([z.string(), z.array(z.string())]).optional().default(''),
      reportTemplateId: z.string(),
      parameterCode: z.string(),
    }),
  ),
  distributionGroups: z.array(z.string()).optional(),
})
  .refine(
    (data) => data.forDuration !== 'last' || (data.numberOfDuration && data.numberOfDuration.trim() !== ''),
    {
      path: ['numberOfDuration'],
      message: 'Required',
    })
  .refine(
    (data) => data.repeatCount === 'notrepeat' || (data.repeatInterval && data.repeatInterval.trim() !== ''),
    {
      path: ['repeatInterval'],
      message: 'Required',
    },
...
const ScheduleReportForm = ({ onSuccess }: ScheduleReportFormProps) => {
  ...
  const onSubmit: SubmitHandler<ScheduleTemplateSchemaType> = async (data) => {
    try {
      const beginDate = new Date(data.beginOn.year, data.beginOn.month - 1, data.beginOn.day);
      ...
      const formattedJobData = formatJobData(cronScheduleDefinition, selectedTemplate);
      const runScheduleJobResponse = await runScheduleReportAction(formattedJobData);
      ...
      const formattedScheduleData = {
        ...
        terminateOn: data.terminateOn ? formatDateToISOString(data.terminateOn) : null,
        parameters: updatedParameters,
        distributionGroup: [],
        jobId:runScheduleJobResponse.data.id,
      };      
      
      const schedulingResponse = await addScheduleReportAction(formattedScheduleData);
      if (schedulingResponse?.state === 'success') {
        toast.success('Report Scheduled Successfully');
        onSuccess?.();
      } else {
        toast.error(schedulingResponse?.error ?? 'Oops! Schedule report failed');
      }
      } else {
