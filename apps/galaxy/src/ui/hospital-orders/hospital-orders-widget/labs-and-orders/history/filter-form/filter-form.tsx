...
const schema = z
  ...
  .superRefine((data, ctx) => {
    if (
      ...
      data.historyCreatedTo < data.historyCreatedFrom
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'To date must be after from date',
        path: ['historyCreatedTo'],
      })
    }
  })
...
const FilterForm = ({ patientId, sectionName }: FilterFormProps) => {
  const { fetchLabAndOrdersHistories } = useStore((state) => ({
  ...
  const onSubmit: SubmitHandler<SchemaType> = async (data) => {
    return fetchLabAndOrdersHistories(patientId, sectionName, data)
  }
