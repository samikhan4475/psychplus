...
import { updateServiceGroupAction } from '../actions'
import { GROUP_STATUSES } from '../constants'
import { HxListTable } from '../hx-list-table'
import { useStore } from '../store'
import { ServiceGroup } from '../types'

const ServiceGroupHistoryDialog = ({ row }: PropsWithRow<ServiceGroup>) => {
  ...
  const onStatusChange = async (value: string) => {
    setLoading(true)
    setStatus(value)
    const response = await updateServiceGroupAction(
      {
        ...row.original,
...
export { ServiceGroupHistoryDialog }
