...
import {
  ...
  getStaffById,
} from '../../claim-detail-tab/actions'
import { SearchProcedureCodes } from '../../claim-detail-tab/charges-section'
...
const ProcedureCell: React.FC<ProcedureCellProps> = ({ rowIndex }) => {
  const form = useFormContext<ClaimAddSchemaType>()
  const { selectedStaffData, setSelectedStaffData } = useRevCycleDataProvider()

  const handleProcedureCodeSelected = useCallback(
    async (selectedItem: CodeItem) => {
      ...
      form.trigger(`claimServiceLines.${rowIndex}.cptCode`)
    },
    [rowIndex],
  )
  const fetchAndProcessCPTData = useCallback(
    async (
      code: string,
      ...
      if (data.state === 'success') {
        ...
        if (response.length > 0) {
          ...
          if (!staffData && renderingProviderId) {
            const staffResponse = await getStaffById(
              Number(renderingProviderId),
            )
    ...
    [rowIndex, selectedStaffData],
  )

  const getAmountBasedOnHonors = useCallback(
    (
