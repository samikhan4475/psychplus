...
const validatePayment = ({ claimPayment, paymentDetail }: ValidatePaymentParams): string => {
  ...
  for (const [index, serviceLine] of claimPayment.claimServiceLinePayments.entries()) {
    ...
    const sumOfAmounts = totalPaidForServiceLine + +removeNegative(serviceLine.otherPr);
