    ...
    (set, get) => ({
      ...
      fetchAppointments: async (body, page = 1, reset = true) => {
        ...
        const sanitizedBody = sanitizeFormData({
          ...
          lastCoverageDateEnd: lastCoverageDateStart,
          bookedAppointmentTime: requestBody.bookedAppointmentTime
            ? getUtcTime(parseTime(requestBody.bookedAppointmentTime))
            : undefined,
        })
      ...
      fetchUserSetting: async () => {
        const result = await getSelfUserSettings(CategoryValue.ListView)
        ...
        modifiedFields.forEach(([key, value]) => {
          if (map.has(key)) {
            ...
            newSettings.push({
              ...
              categoryValue: CategoryValue.ListView,
              name: key,
              content: getStringifiedValue(value),
            })
          }
