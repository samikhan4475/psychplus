...
const StatusCell = ({ row: { original } }: PropsWithRow<PatientMedication>) => {
  const defaultStatus = original.prescriptionStatusTypeId
    ? original.prescriptionStatusTypeId.toString()
    : ''
  const [selectedValue, setSelectedValue] = useState(defaultStatus)
  const patientId = useParams().id as string
  const isCancelledOrAwaitingApproval =
    selectedValue === PatientPrescriptionStatus.CANCELLED ||
    selectedValue === PatientPrescriptionStatus.AWAITING_APPROVAL

  const updateMedicationStatus = async (value: string) => {
    setSelectedValue(value)
    const selectedName =
      STATUS_CODESET.find((status) => status.value === value)?.label || ''
