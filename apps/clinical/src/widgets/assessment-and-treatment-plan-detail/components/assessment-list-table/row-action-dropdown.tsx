...
import { deleteAssessment } from '@psychplus/assessment-and-treatment-plan/api.client'
...
import { AssessmentDialogWidgetClient } from '@/widgets/assessment-dialog/assessment-dialog-widget.client'
  ...
  const onDelete = async () => {
    try {
      if (data.id !== undefined) {
        await deleteAssessment(patientId, data.id)
      }
      ...
      const updatedAssessmentAndTreatmentPlans =
        assessmentAndTreatmentPlans.map((record) => {
          ...
          const updatedAssessments: PatientAssessment[] =
            record.patientAssessments?.filter(
              (assessment) =>
                assessment.id !== data.id || assessment.uniqId !== data.uniqId,
            ) ?? []
