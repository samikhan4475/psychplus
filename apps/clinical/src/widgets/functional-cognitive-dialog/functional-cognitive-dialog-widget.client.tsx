...
import { FunctionalCognitiveForm } from './components'

const FunctionalCognitiveDialogWidgetClient: React.FC<
  ProblemFormDialogProps
> = ({ isOpen, isEdit = false, closeDialog, data }) => {
  return (
    <Dialog.Root
      open={isOpen}
    ...
    >
      <Dialog.Content className="relative max-w-[720px] rounded-6 p-12 font-bold text-[#151B4A]">
        ...
        <Dialog.Title size="8">
          {isEdit ? 'Edit' : 'Add'} Cognitive / Functional
        </Dialog.Title>
        <FunctionalCognitiveForm
          isEdit={isEdit}
...
export { FunctionalCognitiveDialogWidgetClient }
