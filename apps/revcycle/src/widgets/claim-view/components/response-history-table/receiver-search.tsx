...
import { ReceiverOption } from '../../types'

interface ReceiverSearchProps {
  onReceiverSelect: (patientId: string, patientName: string) => void
  reset: boolean
}

const ReceiverSearch = ({ onReceiverSelect, reset }: ReceiverSearchProps) => {
  const [searchTerm, setSearchTerm] = useState('')
  const [results, setResults] = useState<ReceiverOption[]>([])
  ...
  useEffect(() => {
    const fetchReceivers = async () => {
      if (searchTerm && showSuggestions) {
        try {
          const data = await getReceivers({ receiverName: searchTerm })
          const mappedResults = data.map((receiver) => ({
          ...
          <ul className="bg-white absolute top-full z-50 max-h-60 w-full overflow-auto rounded-2 bg-[#FFF] px-1 shadow-3">
            {results.map((receiver, index) => (
              <li
                ...
                onMouseDown={() =>
                  handleClick(index, receiver.id, receiver.name)
                }
              >
                <Button
                  tabIndex={0}
                ...
                >
                  <Text size="2">{receiver.name}</Text>
                </Button>
              </li>
            ))}
          </ul>
