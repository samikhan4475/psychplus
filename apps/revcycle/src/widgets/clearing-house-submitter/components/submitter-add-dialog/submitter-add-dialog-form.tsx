...
import {
  SubmitterFormSchema,
  submitterFormSchema,
} from '@/widgets/clearing-house-submitter/schema/submitterForm.schema'
...
const SubmitterAddDialogForm = (props: SubmitterAddDialogFormProps) => {
  ...
  const onSubmit: SubmitHandler<SubmitterFormSchema> = async (data) => {
    try {
      const receiverName = receiversList.find(
        (r) => r.id === data.receiverId,
      )?.receiverName
      const practiceName = practiceList.find(
        (r) => r.id === data.practiceId,
      )?.displayName
  ...
  return (
    <Form form={form} onSubmit={onSubmit}>
      <Flex direction="column" gap="2" px="1">
        ...
        <Flex gap="2" my="1">
          <Box className="flex-1">
            <FormTextInput label="Phone" {...form.register('phone')} />
          </Box>
          <Box className="flex-1">
            <FormTextInput label="Fax" {...form.register('fax')} />
          </Box>
          <Box className="flex-1">
            <FormSelect
              label="Receiver"
        ...
        <AddressComponent form={form} title="Primary Address" />
        <Flex gap="2" my="1">
          <Box className="flex-1">
            <FormTextInput
              label="City"
          ...
          <Box className="flex-1">
            <FormSelect
              ...
              required={true}
              {...form.register('state')}
              options={statesList.codes.map((r) => ({
                label: r.displayName,
                value: r.displayName,
              }))}
            />
